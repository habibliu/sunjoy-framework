...
public class PostgreSqlHelper implements DialectHelper {
	...
	public static String getCountString(String querySelect) {
		...
		if (select.toLowerCase().indexOf("select distinct") != -1
			...
			return new StringBuffer(querySelect.length()).append("select count(1) ").append(TOTAL_RECORD_NUM)
					.append(" from (").append(querySelect.substring(0, orderIndex)).append(" ) t").toString();
		} else {
			return new StringBuffer(querySelect.length()).append("select count(1) ").append(TOTAL_RECORD_NUM)
					.append(" ").append(querySelect.substring(formIndex, orderIndex)).toString();
		}
	}
